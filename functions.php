...
wp_enqueue_script( 'myscript', get_template_directory_uri() . '/dist/js/myscript.min.js' );
} 
...
function twentyseventeen_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Регистрация', 'twentyseventeen' ),
		'id'            => 'account',
		'description'   => __( 'Регистрация', 'twentyseventeen' ),
	) );
	register_sidebar( array(
		'name'          => __( 'Вход', 'twentyseventeen' ),
		'id'            => 'auth',
		'description'   => __( 'Вход', 'twentyseventeen' ),
	) );
}
...
function db_connect() {
	/*require_once dirname( __FILE__ ) . '/tmdb_v3-PHP-API--master/tmdb-api.php';

	$tmdb = new TMDB(); 

	$tmdb->setAPIKey('c4d2f6be1651bb7c2b7695dca45a5bdb');*/
...
/* Динамическое добавление элементов в список select в ACF */
function acf_load_field_genres( $field )
{
	// Reset choices
	$field['choices'] = array();
 	
	// получение массива жанров из БД
	$conn = db_connect();
    $choices = get_genres($conn);

    //заполнение поля в ACF
    if( is_array($choices) ) {
...
add_filter('acf/load_field/name=genres', 'acf_load_field_genres');

function acf_load_field_actors( $field )
{
	// Reset choices
	$field['choices'] = array();
...
function rate_movie() {
	...
	$conn = db_connect();
	$sqlSetStars = "INSERT INTO users_movies (id_user, id_list, id_movie, rate, rateDate) VALUES('". $userID ."', '1', '". $id ."', '". $rate ."', '". $date ."')";
 ...
 */
require get_parent_theme_file_path( '/inc/get_user_info.php' );

/**
 * Функции получения данных фильмов из БД
 */
require get_parent_theme_file_path( '/inc/get_movie_info.php' );

/**
 * Функция добавления фильмов
 */
require get_parent_theme_file_path( '/inc/fill_with_posts.php' );
 ...
 * Комментарии
 */
require get_parent_theme_file_path( '/inc/my-comments.php' );
